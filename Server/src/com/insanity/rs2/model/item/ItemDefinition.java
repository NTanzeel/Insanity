package com.insanity.rs2.model.item;
	...
	public static void init() throws IOException {
		...
		try {
			...
			for(int i = 0; i < count; i++) {
				String name = ItemDefinition.readString(buffer);
				String examine = ItemDefinition.readString(buffer);
				boolean noted = buffer.get() == 1;
				int parentId = buffer.getShort() & 0xFFFF;
				if(parentId == 65535) {
					parentId = -1;
				}
				boolean noteable = buffer.get() == 1;
