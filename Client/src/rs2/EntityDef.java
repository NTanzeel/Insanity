package rs2;// Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
...
public final class EntityDef {
    ...
    public static EntityDef forID(int i) {
        for (int j = 0; j < 20; j++)
            if (cache[j].type == (long) i) return cache[j];
        anInt56 = (anInt56 + 1) % 20;
        EntityDef entityDef = cache[anInt56] = new EntityDef();
        stream.currentOffset = streamIndices[i];
        entityDef.type = i;
        entityDef.readValues(stream);
        //entityDef.readValues(true, aClass30_Sub2_Sub2_60); // Collect original rs2.NPC data
    /*if(i == 2550) { // rs2.NPC ID
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
    entityDef.models = new int[2]; // Number of models the rs2.NPC uses
    int changedColors = 1; // Number of model colors changed on the rs2.NPC
    ...
    entityDef.name = "General Graardor"; // rs2.NPC name
	...
	entityDef.aByte68 = 4;
    entityDef.description = "The mighty lord Bandos!".getBytes(); // rs2.NPC description
	}
	if(i == 2551) { // rs2.NPC ID
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
    entityDef.models = new int[2]; // Number of models the rs2.NPC uses
    int changedColors = 1; // Number of model colors changed on the rs2.NPC
    ...
    entityDef.name = "Sergeant Strongstack"; // rs2.NPC name
    entityDef.combatLevel = 141; //combat level
	entityDef.walkAnim = 6152;
	entityDef.standAnim = 6153;
	entityDef.aByte68 = 1;
    entityDef.description = "A follower of Bandos.".getBytes(); // rs2.NPC description
	}
	if(i == 2552) { // rs2.NPC ID
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
    entityDef.models = new int[2]; // Number of models the rs2.NPC uses
    int changedColors = 1; // Number of model colors changed on the rs2.NPC
    ...
    entityDef.name = "Sergeant Steelwill"; // rs2.NPC name
	...
	if(i == 2553) { // rs2.NPC ID
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
    entityDef.models = new int[1]; // Number of models the rs2.NPC uses
    int changedColors = 1; // Number of model colors changed on the rs2.NPC
    entityDef.models[0] = 27795; // 27789
    entityDef.name = "Sergeant Grimspike"; // rs2.NPC name
    entityDef.combatLevel = 142; //combat level
	entityDef.walkAnim = 6152;
	entityDef.standAnim = 6153;
	entityDef.aByte68 = 1;
	...
	if(i == 2554) { // rs2.NPC ID
    //aClass30_Sub2_Sub2_60.anInt1406 = streamIndices[630];
    //entityDef.readValues(true, aClass30_Sub2_Sub2_60); // Collect original rs2.NPC data
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
	entityDef.aByte68 = 5;
    entityDef.models = new int[5]; // Number of models the rs2.NPC uses
    int changedColors = 1; // Number of model colors changed on the rs2.NPC
    entityDef.originalModelColors = new int[changedColors];
    entityDef.modifiedModelColors = new int[changedColors];
    entityDef.originalModelColors[0] = 61; // Original model color
    ...
    entityDef.name = "K'ril Tsutsaroth"; // rs2.NPC name
    entityDef.combatLevel = 650; //combat level
	entityDef.anInt91 = 110; //width?
	entityDef.anInt86 = 110; //height?
	entityDef.walkAnim = 6942;
	entityDef.standAnim = 6943;
    entityDef.description = "The mighty lord zamoraks demon K'ril Tsutsaroth!".getBytes(); // rs2.NPC description
	}
	if(i == 2555) { // rs2.NPC ID
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
    entityDef.models = new int[4]; // Number of models the rs2.NPC uses
    entityDef.models[0] = 27775; // 27789
	entityDef.models[1] = 27767;
	entityDef.models[2] = 27763;
	entityDef.models[3] = 17390;
    entityDef.name = "Tstanon Karlak"; // rs2.NPC name
    entityDef.combatLevel = 145; //combat level
	entityDef.walkAnim = 63;
	entityDef.standAnim = 66;
	entityDef.aByte68 = 3;
    entityDef.description = "A follower of Zamorak.".getBytes(); // rs2.NPC description
	}
	if(i == 2556) { // rs2.NPC ID
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
    entityDef.models = new int[4]; // Number of models the rs2.NPC uses
	...
	entityDef.models[3] = 17390;
    entityDef.name = "Zakl'n Gritch"; // rs2.NPC name
    entityDef.combatLevel = 142; //combat level
	entityDef.walkAnim = 63;
	entityDef.standAnim = 66;
	...
	entityDef.models[3] = 17390;
    entityDef.name = "Balfrug Kreeyath"; // rs2.NPC name
	...
	if(i == 2558) { // rs2.NPC ID
    //aClass30_Sub2_Sub2_60.anInt1406 = streamIndices[630];
    //entityDef.readValues(true, aClass30_Sub2_Sub2_60); // Collect original rs2.NPC data
    entityDef.actions = new String[5];
    entityDef.actions[1] = "Attack";
	entityDef.aByte68 = 5;
    entityDef.models = new int[3]; // Number of models the rs2.NPC uses
    int changedColors = 1; // Number of model colors changed on the rs2.NPC
    entityDef.originalModelColors = new int[changedColors];
    entityDef.modifiedModelColors = new int[changedColors];
    entityDef.originalModelColors[0] = 61; // Original model color
	...
	entityDef.anInt86 = 110; //height?
    entityDef.name = "Kree'arra"; // rs2.NPC name
    entityDef.combatLevel = 650; //combat level
	entityDef.walkAnim = 3500;
	entityDef.standAnim = 3501;
    entityDef.description = "The mighty lord Armadyls general kree'arra!".getBytes(); // rs2.NPC description
	}
	if (i == 2559) {
    ...
    entityDef.name = "Commander Zilyana"; // rs2.NPC name
    ...
    entityDef.description = "The mighty lord Saradomins commander Zilyana !".getBytes(); // rs2.NPC description
	}
	...
	entityDef.aByte68 = 3;
	entityDef.readValues(stream); // Collect original rs2.NPC data
	entityDef.actions = new String[5];
	entityDef.actions[1] = "Attack";
	entityDef.models = new int[5];
            ...
            for (int j = 0; j < 6386; j++) {
                ...
                if (npc.actions != null) {
                    for (int ii = 0; ii < 5; ii++) {
                        dat.writeByte(30 + ii);
                        if (npc.actions[ii] == null) writeString(dat, "hidden");
                        else writeString(dat, npc.actions[ii]);
                    }
                }
    ...
    public Model method164(int j, int k, int ai[]) {
        ...
        if (model == null) {
            ...
            for (int i1 = 0; i1 < models.length; i1++)
                if (!Model.method463(models[i1])) flag = true;
